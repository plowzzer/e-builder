...
export function templateToMjml(template) {
        ...
        .map((col) => {
          const componentsMarkup = col.components
            .map((comp) => {
              ...
              if (comp.type === "mj-social") {
                let elements = [];
                try { elements = comp.content ? JSON.parse(comp.content) : []; } catch {}
                const elementsMarkup = elements
                  .map((el) => {
                    const elAttrs = [`name="${el.name}"`, `href="${el.href || ""}"`];
                    if (el.src) elAttrs.push(`src="${el.src}"`);
                    if (el.backgroundColor) elAttrs.push(`background-color="${el.backgroundColor}"`);
                    if (el.iconColor) elAttrs.push(`color="${el.iconColor}"`);
                    return `<mj-social-element ${elAttrs.join(" ")}>${el.label || ""}</mj-social-element>`;
                  })
                  .join("\n");
                return `<mj-social ${attrs}>\n${elementsMarkup}\n</mj-social>`;
              }
              return "";
            })
            .join("\n");
  ...
  const fontsMarkup = (globalConfig.fonts || [])
